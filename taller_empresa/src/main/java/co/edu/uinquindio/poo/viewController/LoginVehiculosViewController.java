...
public class LoginVehiculosViewController {
    ...
    ObservableList<CajaCambios> tipocaja = FXCollections.observableArrayList(CajaCambios.MANUAL, CajaCambios.AUTOMATICA);
    ...
    private ComboBox<CajaCambios> comboxCajaCambios; // Value injected by FXMLLoader
    ...
    void AddAuto(ActionEvent event) {
        ...
        if(validacion){
            int respuesta = 0;
            Auto auto = new Auto(placa, marca, modelo, LocalDate.now(), numeroPuertas);
            respuesta = loginVehiculosController.agregarAuto(auto);
            if(respuesta == 1){
    ...
    void AddCamioneta(ActionEvent event) {
        ...
        if(validacion){
            int respuesta = 0;
            Camioneta camioneta = new Camioneta(placa, marca, modelo, LocalDate.now(), capacidadCarga);
            respuesta = loginVehiculosController.agregarCamioneta(camioneta);
            if(respuesta == 3){
    ...
    void AddMoto(ActionEvent event) {
        ...
        CajaCambios cajaCambios = (CajaCambios) comboxCajaCambios.getValue();
        boolean validacion = validarDatos(placa, marca, modelo);
        if(validacion){
            int respuesta = 0;
            Moto moto = new Moto(placa, marca, modelo, LocalDate.now(), cajaCambios);
            respuesta = loginVehiculosController.agregarMoto(moto);
            if(respuesta == 2){
    ...
    void RemoveAuto(ActionEvent event) {
        Vehiculo vehiculo = tblAuto.getSelectionModel().getSelectedItem();
        if(vehiculo != null){
            int respuesta = loginVehiculosController.eliminarAuto(vehiculo);
            if(respuesta == 1){
                System.out.println("Auto eliminado");
                listaAutos.remove((Auto)vehiculo);
                limpiarseleccionAuto(vehiculo);
            }
        }
    }
    ...
    void RemoveCamioneta(ActionEvent event) {
        Vehiculo vehiculo = tblCamioneta.getSelectionModel().getSelectedItem();
        if(vehiculo != null){
            int respuesta = loginVehiculosController.eliminarCamioneta(vehiculo);
            if(respuesta == 3){
                System.out.println("Camioneta eliminada");
                listaCamionetas.remove((Camioneta)vehiculo);
                limpiarseleccionCamioneta(vehiculo);
            }
        }
    }
    ...
    void RemoveMoto(ActionEvent event) {
        Vehiculo vehiculo = tblMoto.getSelectionModel().getSelectedItem();
        if(vehiculo != null){
            int respuesta = loginVehiculosController.eliminarMoto(vehiculo);
            if(respuesta == 2){
                System.out.println("Moto eliminada");
                listaMotos.remove((Moto)vehiculo);
                limpiarseleccionMoto(vehiculo);
            }
        }
    }
    ...
    void UpdateAuto(ActionEvent event) {
        try{
            ...
            if(validacion){
                Auto auto = new Auto(placa, marca, modelo, LocalDate.now(), numeroPuertas);
                int respuesta = loginVehiculosController.actualizarAuto(auto, placa);
                if(respuesta == 1){
                    System.out.println("Auto actualizado");
                    listaAutos.set(listaAutos.indexOf((Auto)vehiculoseleccionado), auto);
                    limpiarseleccionAuto(auto);
                }
            }
        }catch(Exception e){
    ...
    void UpdateCamioneta(ActionEvent event) {
        try{
            ...
            if(validacion){
                Camioneta camioneta = new Camioneta(placa, marca, modelo, LocalDate.now(), capacidadCarga);
                int respuesta = loginVehiculosController.actualizarCamioneta(camioneta, placa);
                if(respuesta == 3){
                    System.out.println("Camioneta actualizada");
                    listaCamionetas.set(listaCamionetas.indexOf((Camioneta)vehiculoseleccionado), camioneta);
                    limpiarseleccionCamioneta(camioneta);
                }
            }
        }catch(Exception e){
    ...
    void UpdateMoto(ActionEvent event) {
        try{
            ...
            if(validacion){
                Moto moto = new Moto(placa, marca, modelo, LocalDate.now(), cajaCambios);
                int respuesta = loginVehiculosController.actualizarMoto(moto, placa);
                if(respuesta == 2){
                    System.out.println("Moto actualizada");
                    listaMotos.set(listaMotos.indexOf((Moto)vehiculoseleccionado), moto);
                    limpiarseleccionMoto(moto);
                }
            }
        }catch(Exception e){
    ...
    public void setApp(App app) {
        this.app = app;
        loginVehiculosController = new LoginVehiculosController(app.getEmpresa());
        initView();
    }
    ...
    private void initDataBindingMoto(){
        ...
        columnCajaCambios.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getCajaCambios().toString()));
    }
    ...
    private void mostrarInformacionMoto(Moto moto) {
        if (moto != null) {
            ...
            comboxCajaCambios.setValue(moto.getCajaCambios());
        } else {
            VaciarCampos(moto);
        }
    }
    ...
    private void MostrarWidgetsSeleccion(Vehiculo vehiculo){
        if(vehiculo instanceof Auto){
            ...
            comboxCajaCambios.setVisible(true);
        }
    }
    ...
    private void VaciarCampos(Vehiculo vehiculo){
        if(vehiculo instanceof Auto){
            ...
            txtMarcaMoto.clear();
            txtModeloMoto.clear();
            comboxCajaCambios.setValue(null);
        }
    }
    ...
    private void ocultarWidgets(){
        ...
        comboxCajaCambios.setVisible(false);
    }
